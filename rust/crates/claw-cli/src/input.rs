...
impl LineEditor {
    ...
    fn move_up(session: &mut EditSession) {
        ...
        session.cursor = move_vertical(&session.text, session.cursor, -1_i8);
    }
...
fn move_vertical(text: &str, cursor: usize, delta: i8) -> usize {
    ...
    let max_row = starts.len().saturating_sub(1);
    let target_row = match delta {
        -1 => current_row.saturating_sub(1),
        1 => (current_row + 1).min(max_row),
        _ => current_row,
    };
