...
mod tests {
    ...
    fn request_translation_uses_openai_compatible_shape() {
        ...
        assert!(
            payload["messages"][2].get("is_error").is_none(),
            "is_error must not be sent to OpenAI — it is not part of the spec"
        );
    }

    #[test]
    fn assistant_text_only_does_not_include_empty_tool_calls_array() {
        let payload = build_chat_completion_request(&MessageRequest {
        ...
        assert_eq!(assistant["content"], json!("Olá!"));
        assert!(
