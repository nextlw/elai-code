...
pub const ELAI_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".elai.json"),
            |parent| parent.join(".elai.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".elai.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".elai").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".elai").join("settings.local.json"),
            },
        ]
    }
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".elai.json");
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".elai");
    ...
    #[test]
    fn loads_and_merges_elai_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".elai");
        fs::create_dir_all(cwd.join(".elai")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".elai.json"),
        ...
        assert_eq!(ELAI_SETTINGS_SCHEMA_NAME, "SettingsSchema");
